...
import { useGetEmployeeConveyanceSummaryQuery } from 'store/api/report/reportApi';
import {
  Autocomplete,
  Box,
  Grid,
...
import PrintEmployeeWiseSummary from './PrintEmployeeWiseSummary';
import EmployeeWiseSummaryRow from './EmployeeWiseSummaryRow';
...
const EmployeeWiseSummary = () => {
  ...
  const { data, isLoading } = useGetEmployeeConveyanceSummaryQuery(
    { ...query },
    { refetchOnMountOrArgChange: true }
  );
  ...
  // calculation
  const totalConveyances = totalSum(allExpenses, 'amount');
  ...
  return (
    <MainCard
      title="Employee Conveyance Summary"
      secondary={
        <Tooltip title="Print">
      ...
      <Table>
        <TableHead>
          <TableRow className="row">
            ...
            <StyledTableCellWithBorder>EMPLOYEE</StyledTableCellWithBorder>
        ...
        <TableBody>
          {allExpenses?.length ? (
            allExpenses?.map((el, index) => (
              <EmployeeWiseSummaryRow key={index} sn={index + 1} data={el} />
            ))
          ) : (
            <TableRow className="row">
...
export default EmployeeWiseSummary;
