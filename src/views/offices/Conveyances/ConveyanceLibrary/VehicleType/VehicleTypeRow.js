...
import { useDeleteVehicleTypeMutation } from 'store/api/vehicleType/vehicleTypeApi';
import { setToast } from 'store/toastSlice';
import ConfirmDialog from 'ui-component/ConfirmDialog';
import { StyledTableCell, StyledTableRow } from 'ui-component/table-component';
import UpdateVehicleType from './UpdateVehicleType';

const VehicleTypeRow = ({ sn, data }) => {
  ...
  const handleDelete = async () => {
    setDialog(false);
    try {
      const res = await deleteVehicleType(data?.id).unwrap();
      if (res.success) {
        dispatch(
          setToast({
            open: true,
