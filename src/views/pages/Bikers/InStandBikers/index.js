...
import InStandBikerRow from './InStandBikerRow';
...
const InStandBikers = () => {
  ...
  useEffect(() => {
    ...
    axiosPrivate
      .get(
        `/biker?isActive=true&inStand=true&page=${page + 1}&size=${rowsPerPage}`
      )
      .then((res) => {
          ...
          <TableBody>
            {filterData?.length ? (
              filterData
                ...
                .map((item) => (
                  <InStandBikerRow key={item.id} sn={sn++} data={item} />
                ))
            ) : (
              <StyledTableRow>
