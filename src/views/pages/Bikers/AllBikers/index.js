...
import BikerRow from './BikerRow';
import AddBiker from './AddBiker';
...
const Bikers = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axiosPrivate
      .get(`/biker?isActive=true&page=${page + 1}&size=${rowsPerPage}`)
      .then((res) => {
  ...
  return (
    ...
    >
      ...
      <AddBiker open={open} handleClose={() => setOpen(false)} />
      {/* end popup items */}
        ...
        <Table sx={{ minWidth: 400 }}>
          <TableHead>
            <StyledTableRow>
              ...
              <StyledTableCell>Bike Reg. No</StyledTableCell>
              <StyledTableCell>Bike Photo</StyledTableCell>
          ...
          <TableBody>
            {filterData?.length ? (
              filterData
                ...
                .map((item) => <BikerRow key={item.id} sn={sn++} data={item} />)
            ) : (
              <StyledTableRow>
