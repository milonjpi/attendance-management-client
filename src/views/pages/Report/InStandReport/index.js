...
import InStandReportRow from './InStandReportRow';
...
const InStandReport = () => {
  ...
  useEffect(() => {
    ...
    axiosPrivate
      .get(
        `/standLog?inStand=true&page=${
          page + 1
        }&size=${rowsPerPage}&sortBy=inTime`
      )
      .then((res) => {
          ...
          <TableBody>
            {filterData?.length ? (
              filterData
                ...
                .map((item) => (
                  <InStandReportRow key={item.id} sn={sn++} data={item} />
                ))
            ) : (
              <StyledTableRow>
