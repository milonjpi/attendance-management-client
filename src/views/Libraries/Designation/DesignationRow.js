...
import { useDeleteDesignationMutation } from 'store/api/designation/designationApi';
...
const DesignationRow = ({ sn, data }) => {
  ...
  const handleDelete = async () => {
    setDialog(false);
    try {
      const res = await deleteDesignation(data?.id).unwrap();
      if (res.success) {
        dispatch(
          setToast({
            open: true,
